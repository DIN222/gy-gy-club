/* . [BLOCK: SHARED_LOGIC_v4.2_PURE_ORIGINAL] */
const GY_LANGS = [
...
function initSharedAttributes() {
    ...
    // 2. Создаем контейнер (в точности как был на 1-й странице)
    ...
    // Смещение языка только если есть стрелка
    ...
    // Если на странице уже был старый интерфейс — заменяем его, чтобы не было дублей
    const existing = document.getElementById('gy-header');
    if (existing) {
    ...
    // Звук только для второй страницы

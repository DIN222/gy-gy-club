...
Глобальный стандарт навигационных стрелок (Шапка / Угол):

Расположение и позиционирование: Строго фиксированный блок (position: fixed; top: 18px; left: 20px; z-index: 9999999;).

Типографика и стиль: Шрифт Share Tech Mono, размер 26px, толщина 900, цвет --gold, межбуквенный интервал letter-spacing: -2px, с обязательной черной обводкой текста (text-shadow) для контраста.

Элементы внутри блока (.arrow-item):

Одиночная стрелка (←): Возврат на предыдущую страницу (history.back()).

Двойная стрелка (⇐): Переход ко входу в Клуб / Главный Холл (hall.html).

Интерактив: Плавное изменение прозрачности при наведении (transition: opacity 0.2s;, на hover — opacity: 0.8;).

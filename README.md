## 🌌 Концепция интерактивности и атмосферы (GY-GY Club v5.0+)

Проект уходит от старой концепции плоских страниц в сторону «живого» интерактивного пространства. Вместо тяжелых видео-фонов, замедляющих работу устройств, мы используем принцип **точечной магии и легкого динамизма**:

1. **Производительность превыше всего:** Базовый каркас, сетка и логика остаются быстрыми и легкими (чистый CSS/JS), без перегрузки тяжелыми медиафайлами.
2. **Атмосферные акценты (Слои динамики):**
   - **Визуальные эффекты:** Легкие CSS/Canvas-анимации (туман, дым, микро-пружинящие кнопки при клике) вместо тяжелых видео.
   - **Звуковой дизайн (Web Audio API):** Переход от монотонного фонового шума к многослойной звуковой палитре (эмбиент бара с редкими случайными звуками + интерактивные триггеры).
3. **Реальное время:** Интеграция с Cloudflare Workers для живых ответов Мастера Юмора, привязанных ко времени суток и контексту клуба.

---

### 🗺️ План поэтапной реализации динамики

#### Этап 1: Аудио-дизайн и микроинтерактив кнопок (Текущий шаг)
- [ ] Добавить «барные» звуковые эффекты при кликах (стук дерева, звон бокалов, шуршание).
- [ ] Оживить ключевые элементы интерфейса (двери, кнопки входа) легкими CSS-анимациями (хлопки, дрожание, пружина).
- [ ] Настроить плавные переходы между состояниями сайта.

#### Этап 2: Атмосферные визуальные эффекты
- [ ] Внедрить легкий анимированный дым/туман на CSS/Canvas в зоны ожидания или приветствия.
- [ ] Оптимизировать отображение мобильной версии, чтобы анимации не нагружали процессор смартфонов.

#### Этап 3: Интерактив реального времени (ИИ и Скрипты)
- [ ] Расширить логику Cloudflare Worker, чтобы Мастер Юмора учитывал время суток пользователя при выдаче реплик.
- [ ] Добавить динамические звуковые триггеры в момент генерации ответа ИИ (аплодисменты, дзинь бокала).MANIFEST & ARCHITECTURE OF CLUB v4.6.1 CORE
